...
class AssetBrowser(rend.Page, forms.ResourceMixin):
    ...
    def render_navigationform(self, ctx, data):
        ...
        ctx.tag.fillSlots('contentType','cms')
        return ctx.tag
...
class DefaultImageFactory(object):
    ...
    def getAltStr(self, asset):
        return ' '.join( asset.shortDescription.get('en', '').splitlines() ).replace("\\","\\\\").replace("'","\\'")
    ...
    def getImages(self, avatar, storeSession, query):
        ...
        def findImages(storeSession):
            ...
            where = """ item.id in (
                ...
                from
                    file f
                    join content_item ci on f.id = ci.id
                where
            ...
            d = cm.findMany(storeSession, where=where)
