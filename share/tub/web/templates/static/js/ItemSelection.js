...
Cms.Forms.ItemSelection.popupForReST = function(elementId,types,formatTypeId) {
    var url = '/content/_itemselector_?element_id='+escape(elementId)
    if(types!='') {
      url = url +'&types='+escape(types);
    }
    ...
    popup.callback = function(formValues) {
      ...
      if (formatTypeId == 'markdown') {
        var tag = 'cmsitemselection://' + formValues.toString() + "\n\n";
      }
      if (formatTypeId == 'rest') {
